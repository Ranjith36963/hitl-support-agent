...
1. **Stop the false "Bitext" claim (highest priority — it is a live false
   ...
   - ✅ (a) Relabel honestly everywhere: "evaluated on 10 hand-curated tickets,
     one per code path" — true and defensible, just less impressive.
...
3. ✅ **Correct the docs.** **DONE** — `CLAUDE.md` test count fixed 118 → 136
   ...
   (see item 4).
4. ✅ **Decide v3's fate honestly.** **DONE** — kept both paths, v3 stays the
   default, and the `src/graph.py` comment was rewritten from "v3 scheduled for
   deletion in v4.1" to "both paths retained intentionally; v3 won the
   head-to-head; do not delete or flip the default." The README "v4 trades a
   small drop in escalation precision" line was replaced with "v4 did not beat
   v3."
5. ⬜ **Resolve the half-built eval scaffolding** — `eval/ab_model_swap.py` and
   `eval/multiagent_evaluators.py`: run them or remove them. **STILL OPEN.**
