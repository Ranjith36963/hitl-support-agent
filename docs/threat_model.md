...
| Account hijack via App Password leak | Spoofing / Elevation | Gmail App Passwords are scoped to a single app and revocable in Gmail Settings; we recommend per-deploy distinct passwords. | Medium. | Google is phasing out App Passwords in favour of OAuth2; we have not migrated. **OAuth2 is MISSING.** |
